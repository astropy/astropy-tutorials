...
var Search = {
  ...
  makeSearchSummary : function(text, keywords, hlwords) {
    ...
    // Find the first image in the tutorial and use that as a thumbnail for
    // the search results.
    // TODO: in the future, we may want to make the thumbnail image a
    // customizable setting for each tutorial
    var imageLineIndex = textLower.indexOf('.. image::');
    if (imageLineIndex != -1) {
      var imageLineEndIndex = textLower.indexOf("\n", imageLineIndex);
      var imageLine = text.substr(imageLineIndex,
        imageLineEndIndex-imageLineIndex);
      var imageName = imageLine.split('nboutput/')[1];
      var imageURL = "_images/" + imageName;
    } else {
      var imageURL = "_static/default_thumbnail.png";
    }

    function cleanupRSTString(rstString) {
      var newStr;

      // Replace continuous whitespace with single whitespace
      newStr = rstString.replace(/(\s{2,})/gm, " ");
      newStr = newStr.replace(/(\r\n|\n|\r)/gm, " ");

      // Replace heading underlines and newlines:
      newStr = newStr.replace(/(\s[=]{3,}\s)/gm, ": ");
      newStr = newStr.replace(/(\s[\-]{3,}\s)/gm, ": ");

      // Remove Sphinx directives:
      newStr = newStr.replace(/(.. [a-zA-Z0-9\-]+::)/gm, "");
      newStr = newStr.replace(/(:[a-zA-Z0-9\-]+:)/gm, "");

      // Remove URLs:
      newStr = newStr.replace(/(?:www|https?)[^\s]+/gm, "");
    ...
    // Start looking at search text below the main header
    ...
    if (hlwords == "filtertutorials") {
      ...
      // Find the "Summary" header and text below
      var summaryHdrIdx = textLower.indexOf("summary", startIdx);
      var summaryTextIdx = textLower.indexOf("---\n\n", summaryHdrIdx) + 5;

      if (summaryHdrIdx > -1) {  // Summary header found, extract text
        var parBreakIdx = textLower.indexOf('\n\n', summaryTextIdx);
        // TODO: this could be customizable - I set it to an arbitrary value...
        var defaultSummaryLength = 512;
        var summaryLen = Math.min(defaultSummaryLength,
                                  parBreakIdx - summaryTextIdx);
        excerpt = cleanupRSTString(text.substr(summaryTextIdx, summaryLen));
        excerpt = excerpt + '...';

      } else {  // No summary header found
      ...
      $.each(keywords, function(i, x) {
        ...
        var charReadLen = 200; // Grab 200 characters on either side of keyword
        var dispWordNum = 8; // Display words of context on either side of keyw

        // Read a bit of text before, and a bit of text after the keyword:
        var i1 = keyIdx - charReadLen;
        var charReadLen1 = charReadLen;
        var i2 = keyIdx;
        var charReadLen2 = charReadLen;

        if (i1 < startIdx) {
          i1 = startIdx;
          var charReadLen1 = keyIdx - startIdx;
        }
        if ((i2 + charReadLen2) > textLen) {
          var charReadLen2 = textLen - keyIdx;
        }

        var tmp = textLower.substr(i1, charReadLen1);
        // console.log(tmp);
        var tmp1 = cleanupRSTString(tmp);

        tmp = textLower.substr(i2, charReadLen2);
        // console.log(tmp);
        var tmp2 = cleanupRSTString(tmp);

        // Split the string and only keep non-empty elements
        var words = tmp1.split(" ").slice(-dispWordNum);
        words = words.concat(tmp2.split(" ").slice(0, dispWordNum+1));
        words = words.filter(function (x) { return x != null && x != ""; });
        words = words.join(" ");
    ...
    // Now loop through the search keywords and highlight terms that were caught
    $.each(hlwords, function() {
      if (this.search('filter') == -1) {
        // if "filter" is not there in the search query then directly use the
        // query to highlight text
        rv = rv.highlightText(this, 'highlighted');
      } else {
        // if "filter" is there in the search query then first strip "filter"
        // from the query and then use it to highlight text
